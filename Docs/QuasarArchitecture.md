...
- Dashboard server view selection now persists in browser local storage while still accepting `?view=list` / `?view=cards` overrides; card and list layouts use the same catalog order by unique name. Dashboard cards expose the assigned config profile as an actionable chip, the server port as a direct-connect copy chip (`host:port`), and the same create-server entry point the list layout already had. Crashed or faulted servers show one latest error line from the Dedicated Server or Magnetar log in the card/list status, and that line opens the console dialog directly in the matching error-excerpt view. The first-setup wizard requires a Dedicated Server world template first, then a config profile generated from that exact template; it skips profile creation only when that relationship already exists, preselects both for server creation, and can be explicitly closed from its final step once Quasar.Agent is connected.
