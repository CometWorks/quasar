...
Bootstrap sets `QUASAR_DATA_DIR` to the launcher install root when the variable
is not already set, so the launcher and worker share one default state root.
