...
- the Entities page exposes plugin extension targets for supported grids and asteroid voxels. The Entity Viewer UI plugin renders the viewer button and fullscreen metadata-only browser dialog. The viewer scene API asks the connected `Quasar.Agent` for a game-thread snapshot over the existing command/result WebSocket path and returns logical model/texture names, active scene mod metadata including Workshop published-file IDs, block placement, transforms, color/build state, generated cube-part references, runtime subpart references, and warnings. Quasar does not serve `.mwm` files, texture files, extracted mesh geometry, or browser renderer assets; the browser viewer must use a user-selected local Space Engineers `Content` folder and, for modded assets, an optional local Mods or Steam Workshop `content/244850` folder. Browser `localStorage` can remember the selected folder names, while reusable folder access depends on browser-supported directory handles and permissions. The browser parses local `.mwm` render mesh tags, follows `GeometryDataAsset` indirection, resolves active Workshop mods by published-file ID when available, and falls back to proxy geometry when assets are missing or parsing fails.
