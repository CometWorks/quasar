...
LCD text uses the real Space Engineers bitmap font data for the supported `Debug` and `Monospace` font IDs. The browser parses `Fonts/white_shadow/FontDataPA.xml` and `Fonts/monospace/FontDataPA.xml` from the selected local `Content` folder, loads the referenced DDS atlas pages locally, decodes those GPU textures once through WebGL readback, and draws glyphs with the game's advance, bearing, line-height, and `144/185` GUI text scale. Other LCD font names currently normalize to the supported `Debug` path because they are color/reskin variants rather than separate first-pass font geometry.
