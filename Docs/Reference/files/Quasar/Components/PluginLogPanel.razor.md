...
**Log table (`MudTable`):** Fixed header, 420 px height, initially sorted by Time (UTC) descending so the first page shows newest entries; columns show Time (UTC), Level, Server, Plugin, and Message with optional exception text in `Color.Error`.
