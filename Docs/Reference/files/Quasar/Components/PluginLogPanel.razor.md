...
## Summary
Reusable panel that displays live plugin log entries from `PluginLogStream`. Provides filter controls for server, log level, time range, row limit, and free-text search. Subscribes to the stream's `Changed` event to refresh automatically as new entries arrive.
...
**Injected services:**
- `PluginLogStream LogStream` — in-memory log ring; provides `Query(PluginLogQuery)` and `GetUniqueNames()`.
...
- `_uniqueNameFilter`, `_levelFilter`, `_textFilter` — active filter strings.
...
- Server `MudSelect` — populated from `LogStream.GetUniqueNames()`.
- Level `MudSelect` — static options: Debug, Info, Warning, Error, Critical.
- Range `MudSelect` — Last 1h / 24h / 7d / All.
- Limit `MudNumericField` — 1 to `PluginLogStream.MaxEntriesPerServer`.
...
**Key methods:**
- `Refresh()` — calls `LogStream.Query(new PluginLogQuery { ... })` with all active filters.
