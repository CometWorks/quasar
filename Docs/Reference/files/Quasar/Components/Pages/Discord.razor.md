...
- **Key UI sections:**
  ...
  - Death Message Templates section: copyable `MagnetarPaths.GetQuasarDeathMessagesPath()` file path plus `MudExpansionPanels` with one panel showing a 2-column grid of `MudTextField` (6 lines each) for eight death categories — Suicide, PvP, Turret, Grid, Oxygen, Pressure, Collision, Accident. Template count chip per category.
