...
- **Filters paper:** server checkbox grid with "Select all"; per-server profiler mode selects using user-facing labels (`Simple, low overhead` = `SafeContinuous`, `Extensive, deep detail` = `DeepContinuous`, `Off`) and showing the connected agent's live mode; an info alert explains that extensive samples appear in the Profiler: Top Grids and Profiler: Entity Types panels; grid controls (`Columns`, `Rows`, `Row height`) plus "Reset panels".
...
- **Profiler mode changes:** `HandleProfilerModeChanged` normalises the selected protocol value, persists it to `DedicatedServerCatalog` when the server definition exists, and sends `ServerCommandType.SetProfilerMode` through `AgentRegistry.SendCommandAndWaitAsync` for connected agents.
