...
- **UI sections:**
  ...
  - Colors section: `MudTabs` (Light / Dark) → `RenderPaletteEditor` render fragment that iterates seven `ColorGroup`s (Identity, Surfaces, Hover, Navigation, Text, Lines, Status) and renders a `MudColorPicker` per field using `ColorPickerView.Spectrum`, wrapped in a colour-backed preview field with automatic dark/light foreground contrast.
...
- **Key methods:**
  ...
  - `GetColorPreviewStyle(string)` — sanitizes the current colour into alpha-capable CSS custom properties used by each picker's preview backing and readable foreground.
  - `GetPreviewTextColor(MudColor)` / luminance helpers — composite transparent colours over white, compare contrast against dark and white text, and choose the readable picker foreground automatically.
...
- **Private types:**
  - `ColorField` record — label, getter `Func<ThemePalette, string>`, setter `Action<ThemePalette, string>`, and MudBlazor output format (hover colors use alpha-capable `HexA`).
...
- `MudColorOutputFormats.Hex` is used for standard palette edits, while hover-list fields use `HexA` so alpha-capable colors such as `#bfbfbfff` round-trip.
