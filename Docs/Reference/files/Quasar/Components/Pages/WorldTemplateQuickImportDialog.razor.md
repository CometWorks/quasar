...
- **Key UI**
  ...
  - Predefined Worlds tab lists discovered installed DS templates with search, Refresh, short relative source/category display, and per-row Add buttons. The table declares an explicit name/source/action `ColGroup` and uses `installed-world-template-*` plus shared action-column classes so the unlabeled rightmost Add column remains visible while long source paths truncate within the dialog width.
