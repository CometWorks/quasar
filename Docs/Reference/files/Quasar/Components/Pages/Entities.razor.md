...
- **Injected services:** `AgentRegistry`, `EntityService`, `IDialogService`, `ISnackbar`
- **Key UI sections:**
  - Toolbar: server selector `MudSelect` (connected agents only), type filter `MudSelect` (All/Grid/Character/Float/Voxel), search `MudTextField`, Refresh button (disabled while loading or no agent selected).
...
- **Key methods:**
  ...
  - `HandleChanged()` — re-renders on `AgentRegistry.Changed`; also re-selects a default agent if the current selection was disconnected.
- **Type filter options:** `TypeOptions` static array with values "All", "Grid", "Character", "Float", "Voxel".
- **Event subscription:** `AgentRegistry.Changed`.
