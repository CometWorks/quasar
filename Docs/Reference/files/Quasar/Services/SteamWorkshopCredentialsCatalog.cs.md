...
| `SaveAsync(credentials, ct)` | Normalizes, protects key, writes JSON atomically, restricts Unix file permissions, updates in-memory state. |
...
- `RestrictCredentialFileAccess(path)` — no-op on Windows; on other platforms calls `File.SetUnixFileMode(path, UserRead|UserWrite)` and logs a warning if the filesystem rejects it
...
## Notes
- Data Protection purpose string is `"Quasar.SteamWorkshopCredentials.v1"` — if the local Data Protection keyring is rotated or replaced, unprotect will fail and the key is cleared with a warning (not silently retained).
