...
| `StartServerAsync(...)` | Guarded by `StartInProgress`; resolves runtime, prepares files, rotates the active stdout/stderr log slot, spawns the process with full env vars (including agent reconnect/shutdown and per-server profiler mode), prepends Linux native-library search paths to `LD_LIBRARY_PATH` when the resolver found them, optionally logs launch diagnostics when the server definition enables `LogLaunchEnvironment`, applies startup priority, starts stdout/stderr pumps. |
