...
| `StartServerAsync(...)` | Guarded by `StartInProgress`; resolves runtime, prepares files, rotates the active stdout/stderr log slot, spawns the process with full env vars, prepends Linux native-library search paths to `LD_LIBRARY_PATH` when the resolver found them, applies startup priority, starts stdout/stderr pumps. |
