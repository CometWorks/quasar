...
`ManagedRuntimeWarmupService` is a `BackgroundService` that immediately checks and prepares the managed SteamCMD, Magnetar, and Space Engineers Dedicated Server installs at Quasar startup, so managed launches are blocked until those prerequisites are ready. After the startup warmup, it checks the managed Magnetar install for updates every hour and feeds Magnetar checking/download/install progress into the visible component snapshot. It exposes a component-level `ManagedRuntimeWarmupSnapshot` for dashboard progress display and fires a `Changed` event on every status update.
