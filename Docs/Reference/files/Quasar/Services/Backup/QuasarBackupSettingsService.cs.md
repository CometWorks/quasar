...
## Summary
Singleton store for automatic-backup rules. Persists `backup-settings.json` (`MagnetarPaths.GetQuasarBackupSettingsPath()`) in the Quasar data directory and picks up external edits via a debounced (250 ms) `FileSystemWatcher`, mirroring `BrandingService`.
