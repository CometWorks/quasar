...
- `QuasarConfigOptionKind` (enum) — `Boolean`, `Integer`, `Decimal`, `Text`, `LongText`, `Password`, `KeyValueText`, `SelectInteger`, `SelectText`
...
- `QuasarConfigMetadata` (static class):
  ...
  - `Options` — read-only list of `QuasarConfigOptionDefinition` objects spanning Root and Session scopes, including server password and session settings from the vanilla DS WinForms editor; `BlockTypeLimits` uses `KeyValueText`
  ...
  - Private factory helpers: `RootBool`, `RootInt`, `RootDecimal`, `RootText`, `RootSelectText`, `SessionBool`, `SessionInt`, `SessionDecimal`, `SessionSelect`, `SessionText`
