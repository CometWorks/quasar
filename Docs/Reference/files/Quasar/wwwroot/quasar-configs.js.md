...
## Structure
`window.quasarConfigs` object with eleven methods:
...
| `downloadFile(url, fileName)` | `(string, string?) → void` | Creates a temporary same-page anchor with optional `download` filename and clicks it; used for log downloads so Blazor does not route to a missing page after the file response |
