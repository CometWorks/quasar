...
| `updateSceneBounds(refit = false)` | Recomputes displayed relative-space bounds from rendered grid geometry, context bounds, or rendered voxel geometry for voxel-only scenes, falls back to renderer-provided or transformed metadata bounds when needed, then refreshes the floor grid and sun marker placement. |
