...
| `Dispose()` | Unsubscribes process/session events and character death handlers, clears the `StopCommand` hook, stops the connection, disposes the outbox and bridge, unpatches profiler hooks, nulls references. |
