...
| `TrySendAdminStop()` | Best-effort synchronous `AdminStop` send (≤2 s) while the socket is still open; reads `_socket` (volatile) from the game thread. |
...
## Notes
- Sends are serialized via `_sendLock` to prevent concurrent WebSocket writes (snapshot loop, log flush, and `TrySendAdminStop` can all send).
