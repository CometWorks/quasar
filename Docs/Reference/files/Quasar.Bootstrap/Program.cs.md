...
| `StartAsync` | Creates dirs, downloads an initial UI worker into `ManagedRuntime/WebService/<version>` when no packaged/active worker exists, ensures an active-release pointer exists, activates it, starts a `FileSystemWatcher` on the pointer. |
