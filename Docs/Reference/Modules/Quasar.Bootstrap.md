...
| File | Kind | Summary |
| --- | --- | --- |
| [Quasar.Bootstrap/Program.cs](../files/Quasar.Bootstrap/Program.cs.md) | class | Entry point and core logic for the Quasar launcher. It implements `ensure-running`, `serve`, and `activate-release`; supervises the worker process; validates initial web-release layout; watches the active-release pointer; and self-upgrades only when the primary Quasar release stream has an actually newer normalized release identity. |
