...
Backs up and restores Quasar configuration, whole server data, and world-only data as versioned ZIPs. `QuasarBackupService` builds configuration archives (`data/` + `branding-assets/`), server archives (server definition, DS app data, Magnetar app data, world files including config), and world archives (world files excluding `Sandbox_config.sbc*`). It prefers the latest valid Space Engineers `Backup` snapshot when present so backups can be taken while servers run. `QuasarBackupSettingsService` persists the automatic configuration-backup schedule (`backup-settings.json`) with a debounced file watch, and `AutomaticBackupService` writes scheduled configuration backups into the `Backups` directory and prunes them to the retention count. `BackupCompatibility` and `BackupFormatMigrations` enforce semantic-version restore rules. Download endpoints and DI registration live in [Quasar.Host](Quasar.Host.md); the operator UI is `Backup.razor` in [Quasar.Components](Quasar.Components.md).
...
| [Quasar/Services/Backup/QuasarBackupService.cs](../files/Quasar/Services/Backup/QuasarBackupService.cs.md) | class | Builds and restores Quasar configuration, server, and world ZIP backups. Creates in-memory config archives for download, writes timestamped ZIPs into the `Backups` directory, lists/prunes/deletes them, dispatches restore by manifest kind after a version-compatibility check, uses latest SE world backup snapshots when available, and guards against path traversal/zip slip. |
