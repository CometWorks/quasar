...
| [Quasar/Quasar.csproj](../files/Quasar/Quasar.csproj.md) | project file | MSBuild project file for the Quasar Blazor Server host. Targets `net10.0`, pins default release metadata to `0.1.0`, references `Magnetar.Protocol`, declares the web/auth/UI packages, and includes custom targets to compile `Quasar.Agent` and stage its DLLs plus runtime-specific Harmony DLLs alongside the host output. |
