...
| [Quasar/Services/PluginSdk/PluginLogStream.cs](../files/Quasar/Services/PluginSdk/PluginLogStream.cs.md) | class | In-memory ring buffer of recent plugin log entries keyed by server unique name, plus a static parser for the PluginSdk `QuasarLogSink` JSON stdout format. The supervisor feeds entries parsed from each dedicated server's standard output; Blazor components subscribe to `Changed` and read entries via `GetEntries`, `GetRecent`, or `Query`. Follows the same lock-guarded, event-raising shape as other Quasar runtime services. |
