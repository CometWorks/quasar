...
the terminal. Run `.\Quasar.exe serve` again when you want the UI back.
...
with exit code `0`, so the task completes without restarting it. Start the task
to bring the UI and supervisor back.
